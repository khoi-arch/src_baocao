...
# -*- coding: utf-8 -*-
"""
F4a0 teacher-protected correction calibration.

Protocol:
  1) Split train into train_inner/calibration.
  2) Train baseline CE teacher on train_inner.
  3) Train F4a student on same train_inner:
       CE + correction on teacher-wrong + KL protection on teacher-correct.
  4) Evaluate both on calibration, compute fix/damage.

No official validation is used.
...
def main():
    ...
    ap.add_argument("--out-dir", default="05_test/outputs/F4a0_teacher_protected_correction_calibration")
    ap.add_argument("--combined-zip", default="05_test/outputs/F4a0_teacher_protected_correction_calibration.zip")
    ...
    config = {
        "experiment": "F4a0_teacher_protected_correction_calibration",
        "official_validation_used": False,
        "uses_fixed_hard_pairs": False,
        "uses_l3_or_family_labels_for_training": False,
        "method": "Train baseline teacher; student CE + teacher-wrong zero-violation correction + teacher-correct KL protection.",
    ...
    teacher_dir = run_train(
        root=root, trainer=trainer, runs_root=runs_root, run_name="baseline_ce_teacher",
        dataset_npz=subset_npz, metadata_json=metadata_json, train_raw=train_inner_raw, val_raw=calib_raw,
        args=args, tpc_enabled=False,
    )
    teacher_ckpt = teacher_dir / "best_model.pt"
    student_dir = run_train(
        root=root, trainer=trainer, runs_root=runs_root, run_name="teacher_protected_correction_student",
        dataset_npz=subset_npz, metadata_json=metadata_json, train_raw=train_inner_raw, val_raw=calib_raw,
        args=args, tpc_enabled=True, teacher_ckpt=teacher_ckpt,
    )

    rows = []
    for run_name, run_dir in [("baseline_ce_teacher", teacher_dir), ("teacher_protected_correction_student", student_dir)]:
        row = read_metrics(run_dir, label_names, val_prefix="calibration")
        row["run_name"] = run_name
        row["run_dir"] = str(run_dir)
        rows.append(row)
    res = pd.DataFrame(rows).sort_values(["calibration_macro_f1", "calibration_accuracy"], ascending=False).reset_index(drop=True)
    res.to_csv(out_dir / "F4a0_results.csv", index=False)

    base_row = res[res["run_name"] == "baseline_ce_teacher"].iloc[0].to_dict()
    method_row = res[res["run_name"] == "teacher_protected_correction_student"].iloc[0].to_dict()
    delta = float(method_row["calibration_macro_f1"] - base_row["calibration_macro_f1"])

    fix_damage = compute_fix_damage(teacher_dir, student_dir, label_names, out_dir, "F4a0")
    proceed = (
        delta > 0
    ...
    decision = {
        ...
        "delta_method_minus_baseline_calibration_macro_f1": delta,
        ...
        "recommendation": "proceed_to_F4a1_full_official_val_once" if proceed else "reject_F4a_teacher_protected_correction",
    }
    (out_dir / "F4a0_decision.json").write_text(json.dumps(decision, indent=2, default=str), encoding="utf-8")

    lines = []
    lines.append("# F4a0 teacher-protected correction calibration\n")
    ...
    (out_dir / "F4a0_report.md").write_text("\n".join(lines), encoding="utf-8")
