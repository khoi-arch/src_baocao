...
# -*- coding: utf-8 -*-
"""
E5 E3-shaped Ordered-pair Top-2 KEEP/SWAP Swapper.
...
E5 makes the action problem explicit:
...
-------
  E5_summary.json
  E5_history.csv
  E5_ordered_pair_thresholds.csv
  E5_ordered_pair_sweep_train.csv
  E5_ordered_pair_sweep_val.csv
  E5_best_per_class_f1.csv
  E5_best_confusion_matrix.csv
  E5_val_predictions_best.csv
  E5_best_pair_fix_damage.csv
  E5_action_metrics.json
"""
...
def import_official_train(train_script: Path):
    ...
    spec = importlib.util.spec_from_file_location("official_07_train_for_e5", str(train_script))
...
def load_shaped_backbone_into_model(model: nn.Module, ckpt_path: Path, device: torch.device) -> Optional[dict]:
    """Load the backbone.* weights from an E3/E3-like wrapper checkpoint into official D3.

    E5 = E3-shaped representation + E4v2 ordered KEEP/SWAP.
    This function keeps the official D3 architecture but replaces its backbone weights
    with weights already fine-tuned by E3 pair/SupCon training.
    """
    if ckpt_path is None or str(ckpt_path).strip() == "":
        return None
    ckpt_path = Path(ckpt_path)
    if not ckpt_path.exists():
        raise FileNotFoundError(f"shaped checkpoint not found: {ckpt_path}")
    ...
    for k, v in sd.items():
        ...
        if nk.startswith("backbone."):
            backbone_sd[nk[len("backbone."):]] = v
        elif nk.startswith("model.backbone."):
            backbone_sd[nk[len("model.backbone."):]] = v
        else:
            direct_sd[nk] = v

    use_sd = backbone_sd if backbone_sd else direct_sd
    if not use_sd:
        raise ValueError(f"No usable backbone weights found in shaped checkpoint: {ckpt_path}")

    missing, unexpected = model.load_state_dict(use_sd, strict=False)
    return {
        "path": str(ckpt_path),
        "source": "backbone_prefix" if backbone_sd else "direct_state_dict",
        "strict": False,
        "loaded_keys": int(len(use_sd)),
        "missing_keys": list(missing),
        "unexpected_keys": list(unexpected),
        "note": "For E3 checkpoints, source should be backbone_prefix. Missing/unexpected keys should be inspected."
    }
...
def save_outputs(out_dir, inp, args, model, train_eval_loader, val_loader, ordered_pair_names, ordered_pair_labels, ckpt_info, shaped_ckpt_info, rep_dim, tap_module_name, history, best_epoch, device):
    ...
    train_sweep.to_csv(out_dir / "E5_ordered_pair_sweep_train.csv", index=False)
    val_sweep.to_csv(out_dir / "E5_ordered_pair_sweep_val.csv", index=False)
    ...
    th_df.to_csv(out_dir / "E5_ordered_pair_thresholds.csv", index=False)
    ...
    val_eval["per_class"].to_csv(out_dir / "E5_best_per_class_f1.csv", index=False)
    val_eval["confusion"].to_csv(out_dir / "E5_best_confusion_matrix.csv")
    val_eval["pair_fix_damage"].to_csv(out_dir / "E5_best_pair_fix_damage.csv", index=False)

    base_per, base_cm = per_class_and_cm(val_arr["y"], val_arr["top1"], inp["label_names"])
    base_per.to_csv(out_dir / "E5_baseline_per_class_f1.csv", index=False)
    base_cm.to_csv(out_dir / "E5_baseline_confusion_matrix.csv")

    action_train = action_metrics(train_arr, selected_thresholds)
    action_val = action_metrics(val_arr, selected_thresholds)
    save_json(out_dir / "E5_action_metrics.json", {"train": action_train, "val": action_val})
    ...
    pred_df = pd.DataFrame({
        ...
        "do_swap": do_swap,
        "e5_pred_id": pred.astype(int),
        "e5_pred_label": [inp["id_to_label"][int(i)] for i in pred],
        "e5_correct": pred == val_arr["y"],
    })
    pred_df["transition"] = "both_wrong"
    pred_df.loc[pred_df["base_correct"] & pred_df["e5_correct"], "transition"] = "both_correct"
    pred_df.loc[(~pred_df["base_correct"]) & pred_df["e5_correct"], "transition"] = "fixed"
    pred_df.loc[pred_df["base_correct"] & (~pred_df["e5_correct"]), "transition"] = "damaged"
    pred_df.to_csv(out_dir / "E5_val_predictions_best.csv", index=False)

    pd.DataFrame(history).to_csv(out_dir / "E5_history.csv", index=False)

    summary = {
        "stage": "E5_e3_shaped_ordered_pair_swapper",
        "mode": args.mode,
        "research_position": "attention-only E3-shaped representation + ordered-pair-specific KEEP/SWAP heads and thresholds",
        "tree_usage": "none",
        "fix_over_E4": "E5 combines E3-shaped representation with E4v2 ordered heads. It separates the six ordered top1->top2 decisions and can start from E3 backbone weights.",
        "references": {
        ...
        "official_checkpoint_load": ckpt_info,
        "shaped_checkpoint_load": shaped_ckpt_info,
        ...
        "outputs": {
            "history": str(out_dir / "E5_history.csv"),
            "best_model": str(out_dir / "E5_best_model.pt"),
            "thresholds": str(out_dir / "E5_ordered_pair_thresholds.csv"),
            "sweep_train": str(out_dir / "E5_ordered_pair_sweep_train.csv"),
            "sweep_val": str(out_dir / "E5_ordered_pair_sweep_val.csv"),
            "per_class": str(out_dir / "E5_best_per_class_f1.csv"),
            "confusion_matrix": str(out_dir / "E5_best_confusion_matrix.csv"),
            "val_predictions": str(out_dir / "E5_val_predictions_best.csv"),
            "pair_fix_damage": str(out_dir / "E5_best_pair_fix_damage.csv"),
            "action_metrics": str(out_dir / "E5_action_metrics.json"),
        },
    }
    save_json(out_dir / "E5_summary.json", summary)
...
def write_summary_md(out_dir: Path, summary: dict):
    ...
    text = f"""# E5 E3-shaped Ordered-pair Top-2 Swapper
...
E4 had one shared swapper. E5 has separate heads and thresholds for:
...
E5 macro-F1  = {summary['final_val_metrics']['macro_f1']:.6f}
delta macro-F1 = {summary['delta_val']['macro_f1']:+.6f}

base accuracy  = {summary['base_val_metrics']['accuracy']:.6f}
E5 accuracy  = {summary['final_val_metrics']['accuracy']:.6f}
```
...
- `E5_action_metrics.json`
"""
    (out_dir / "E5_summary.md").write_text(text, encoding="utf-8")


def main():
    parser = argparse.ArgumentParser(description="E5 ordered-pair-specific top2 residual swapper")
    ...
    print(f"[E5] repo_root={repo_root}", flush=True)
    print(f"[E5] out_dir={out_dir}", flush=True)
    print(f"[E5] mode={args.mode}", flush=True)
    print(f"[E5] device={device}", flush=True)
    print("[E5] no tree. ordered-pair-specific residual action.", flush=True)
    ...
    print(f"[E5] official checkpoint loaded: {ckpt_info}", flush=True)

    shaped_ckpt_info = None
    if str(args.shaped_checkpoint).strip():
        shaped_ckpt_info = load_shaped_backbone_into_model(
            backbone,
            resolve_path(args.shaped_checkpoint, repo_root),
            device,
        )
        print(f"[E5] shaped backbone loaded: {shaped_ckpt_info}", flush=True)

    rep_dim, tap_module_name = infer_rep_dim(backbone, val_loader, device, args.tap_module)
    print(f"[E5] tap_module={tap_module_name} rep_dim={rep_dim}", flush=True)
    ...
    weight_df.to_csv(out_dir / "E5_ordered_pair_action_weights.csv", index=False)
    print("[E5] ordered action weights:", weight_df[["ordered_pair_label","n_keep","n_swap","keep_weight","swap_weight","method"]].to_dict(orient="records"), flush=True)
    ...
    save_json(out_dir / "E5_run_config.json", {
        "stage": "E5_e3_shaped_ordered_pair_swapper",
        "mode": args.mode,
        "tree_usage": "none",
        "args": vars(args),
    ...
    if args.mode == "eval_zero":
        print("[E5] eval_zero: no training, all thresholds disabled.", flush=True)
    else:
        print(
            f"[E5] training trainable_params={sum(p.numel() for p in model.parameters() if p.requires_grad):,} "
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if improved:
            ...
            torch.save({
                ...
                "rep_dim": int(rep_dim),
                "tree_usage": "none",
            }, out_dir / "E5_best_model.pt")
        else:
            no_improve += 1

        if epoch == 1 or improved or epoch % int(args.log_every) == 0:
            print(
                f"[E5] ep={epoch:03d} lr={lr_epoch:.3e} "
        ...
        if no_improve >= int(args.patience):
            print(f"[E5] early stop at epoch {epoch}", flush=True)
            break
    ...
    save_outputs(out_dir, inp, args, model, train_eval_loader, val_loader, ordered_pair_names, ordered_pair_labels, ckpt_info, shaped_ckpt_info, rep_dim, tap_module_name, history, best_epoch, device)

    if args.make_zip:
        zip_path = out_dir.with_suffix(".zip")
        zip_dir(out_dir, zip_path)
        print(f"[E5] zipped outputs: {zip_path}", flush=True)

    summary = load_json(out_dir / "E5_summary.json")
    print("[E5] done.", flush=True)
    print(f"[E5] mode={args.mode}", flush=True)
    print(f"[E5] base_macro_f1={summary['base_val_metrics']['macro_f1']:.6f}", flush=True)
    print(f"[E5] macro_f1={summary['final_val_metrics']['macro_f1']:.6f}", flush=True)
    print(f"[E5] net_gain={summary['transition_val_vs_base']['net_gain']}", flush=True)
