...
# -*- coding: utf-8 -*-
"""
F3b0 clean calibration search for OOF-selected hard-pair/family margin loss.
No official validation usage.
"""
...
def main():
    ...
    ap.add_argument("--trainer", default="02_src/07_train_hardmargin.py")
    ap.add_argument("--selected-hard-groups-json", default="05_test/outputs/F3a2_oof_aggregate_overlap_audit/F3a2_train_only_selected_hard_groups.json")
    ap.add_argument("--out-dir", default="05_test/outputs/F3b0_hard_margin_calibration_search")
    ap.add_argument("--combined-zip", default="05_test/outputs/F3b0_hard_margin_calibration_search.zip")
    ap.add_argument("--lambda-grid", default="0.0,0.3,0.5")
    ap.add_argument("--margin-grid", default="0.2,0.5")
    ap.add_argument("--hard-margin-mode", default="pair_family")
    ...
    selected_json = resolve_path(args.selected_hard_groups_json, root)
    ...
    if not selected_json.exists():
        raise FileNotFoundError(f"selected hard groups json not found: {selected_json}")

    selected_obj = json.loads(selected_json.read_text(encoding="utf-8"))
    if bool(selected_obj.get("validation_used", False)):
        raise ValueError("selected hard groups used validation; refusing.")
    ...
    config = {
        "experiment": "F3b0_hard_margin_calibration_search",
        "official_validation_used": False,
        "selection_source": "train_inner/calibration only; hard groups from train-only OOF F3a2",
        "configs": [{"lambda": l, "margin": m} for l, m in cfgs],
        "hard_margin_mode": str(args.hard_margin_mode),
        "selected_hard_groups_json": str(selected_json),
        "selected_hard_groups_summary": {
            "hard_families": len(selected_obj.get("hard_families", [])),
            "hard_pairs": len(selected_obj.get("hard_pairs", [])),
            "hard_pair_families": len(selected_obj.get("hard_pair_families", [])),
        },
    ...
    for lam, margin in cfgs:
        ...
        cmd = [
            ...
            "--sam-rho", "0.0",
            "--hard-margin-groups-json", str(selected_json.relative_to(root) if selected_json.is_relative_to(root) else selected_json),
            "--hard-margin-lambda", str(float(lam)),
            "--hard-margin-margin", str(float(margin)),
            "--hard-margin-mode", str(args.hard_margin_mode),
        ]
        ...
        info_path = run_dir / "hard_pair_margin_info.json"
        if info_path.exists():
            hm = json.loads(info_path.read_text(encoding="utf-8"))
            row["hard_active_rows"] = int(hm.get("active_rows", 0))
            row["hard_active_row_rate"] = float(hm.get("active_row_rate", 0.0))
            row["hard_active_entries"] = int(hm.get("active_entries", 0))
        results.append(row)
        pd.DataFrame(results).to_csv(out_dir / "F3b0_results_partial.csv", index=False)
    ...
    res.to_csv(out_dir / "F3b0_results.csv", index=False)
    ...
    decision = {
        ...
        "recommendation": "proceed_to_F3b1_full_train" if float(best["lambda"]) > 0 and delta > 0 else "reject_hard_margin_or_rethink_grid",
    }
    (out_dir / "F3b0_best_config.json").write_text(json.dumps(decision, indent=2, default=str), encoding="utf-8")

    lines = []
    lines.append("# F3b0 OOF-selected hard-pair/family margin search\n")
    lines.append("## Protocol\n")
    lines.append("```text")
    lines.append("Hard groups are fixed from F3a2 train-only OOF.")
    lines.append("Official validation is not used.")
    lines.append("Train temporary official L1 models on train_inner.")
    lines.append("Select lambda/margin by calibration macro-F1.")
    lines.append("```")
    lines.append("\n## Results\n")
    ...
    lines.append("```")
    (out_dir / "F3b0_report.md").write_text("\n".join(lines), encoding="utf-8")
