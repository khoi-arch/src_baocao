...
def parse_args() -> argparse.Namespace:
    ...
    # D1b isolated test knobs. Keep lambda_pair=0.0 to reproduce official training exactly.
    # When lambda_pair>0, auxiliary pairwise BCE loss is added during train only.
    p.add_argument("--lambda-pair", type=float, default=0.0)
    p.add_argument("--aux-hidden-dim", type=int, default=0, help="0 = Linear(hidden_dim->1); >0 = small MLP head")
...
class FusionAblationTransformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, z_values: torch.Tensor | None = None, *, return_info: bool = False, return_pair_tokens: bool = False):
        if z_values is None:
...
D1A_HARD_PAIRS: List[Tuple[str, str]] = [
    ("Ransomware", "Spyware"),
    ("Ransomware", "Trojan"),
    ("Spyware", "Trojan"),
]
...
def make_d1b_aux_head(hidden_dim: int, aux_hidden_dim: int, aux_dropout: float) -> nn.Module:
    if int(aux_hidden_dim) > 0:
        return nn.Sequential(
            nn.Linear(int(hidden_dim), int(aux_hidden_dim)),
            nn.GELU(),
            nn.Dropout(float(aux_dropout)),
            nn.Linear(int(aux_hidden_dim), 1),
        )
    return nn.Linear(int(hidden_dim), 1)
...
def attach_d1b_aux_heads(
    ...
    *,
    ...
    Unlike D1a, the auxiliary pair heads do not read cls_out directly.
    Each hard pair has:
      - a learned query vector q_pair: [hidden_dim]
      - an attention pooling over token_out: [B,F,H] -> pair_rep [B,H]
      - a binary classifier over pair_rep
    ...
    model.d1b_aux_heads = nn.ModuleDict()

    for key, _ida, _idb, _a, _b in model.d1b_pair_id_names:
        q = nn.Parameter(torch.empty(int(hidden_dim)))
        nn.init.normal_(q, mean=0.0, std=0.02)
        model.d1b_pair_queries[key] = q

        if int(aux_hidden_dim) > 0:
            model.d1b_aux_heads[key] = nn.Sequential(
...
def d1b_pair_attention_logits(model: nn.Module, token_out: torch.Tensor) -> Dict[str, torch.Tensor]:
    """
    token_out: [B,F,H]
    returns: aux logits per pair, each [B]
    """
    if token_out.ndim != 3:
        raise ValueError(f"token_out must be [B,F,H], got {tuple(token_out.shape)}")
    ...
    for key, *_ in model.d1b_pair_id_names:
        ...
        pair_rep = torch.einsum("bf,bfh->bh", alpha, token_out)
        out[key] = model.d1b_aux_heads[key](pair_rep).squeeze(-1)

    return out


def d1b_aux_pairwise_loss(model: nn.Module, token_out: torch.Tensor, y: torch.Tensor):
    """
    Compute D1b auxiliary pair loss from token_out via pair-specific attention.

    For a pair A-vs-B, only samples whose true label is A or B contribute.
    Benign samples do not contribute to any pair loss.
    """
    if not hasattr(model, "d1b_aux_heads") or not hasattr(model, "d1b_pair_queries"):
        return torch.zeros((), dtype=torch.float32, device=y.device), {}

    # Defensive device check for newly attached modules.
    for _k, _h in model.d1b_aux_heads.items():
        _p = next(_h.parameters(), None)
        if _p is not None and _p.device != token_out.device:
            raise RuntimeError(
                f"D1b aux head device mismatch: head={_k}, head_device={_p.device}, token_device={token_out.device}. "
                "Call model.to(device) after attaching d1b modules."
            )

    aux_logits = d1b_pair_attention_logits(model, token_out)
    ...
    for key, ida, idb, a, b in model.d1b_pair_id_names:
        ...
        loss = torch.nn.functional.binary_cross_entropy_with_logits(aux_logits[key][mask], target)
        losses.append(loss)
...
def train_one_epoch_d1b(
    *,
    ...
    """Official train_one_epoch semantics + one extra auxiliary loss.

    This intentionally mirrors 02_src/train_utils.py::train_one_epoch:
      - same X/Z/y loader contract
      - same optimizer.zero_grad/backward/clip/step order
      - only changes model(..., return_pair_tokens=True) and loss = main + lambda_pair * pair
    """
    model.train()
    total_loss = 0.0
    total_main = 0.0
    ...
    for X, Z, y in loader:
        ...
        total_main += float(main_loss.item()) * n
        total_pair += float(pair_loss.item()) * n
        total_n += n

    return {
        "loss": float(total_loss / max(total_n, 1)),
        "main_loss": float(total_main / max(total_n, 1)),
        "pair_loss": float(total_pair / max(total_n, 1)),
    }


def evaluate_d1b_aux_pairs(
    model: nn.Module,
    loader: DataLoader,
    device: torch.device,
    ...
    with torch.no_grad():
        for tokens, values, y in loader:
            ...
            _logits, _cls_out, token_out = model(tokens, values, return_pair_tokens=True)
            aux_logits = d1b_pair_attention_logits(model, token_out)
            ...
            for key, ida, idb, a, b in model.d1b_pair_id_names:
                ...
                collected[key]["y"].extend(yy.tolist())
                collected[key]["score"].extend(ss.tolist())

    rows: List[Dict[str, object]] = []
    for key, ida, idb, a, b in model.d1b_pair_id_names:
        yy = np.asarray(collected[key]["y"], dtype=int)
        ss = np.asarray(collected[key]["score"], dtype=float)
        ...
        if len(yy) == 0:
            row.update({"accuracy": None, "macro_f1": None, "auc": None})
        else:
            pp = (ss >= 0.0).astype(int)
            row["accuracy"] = float(accuracy_score(yy, pp))
            row["macro_f1"] = float(f1_score(yy, pp, average="macro", zero_division=0))
            try:
...
def main() -> None:
    ...
    config_obj = {
        ...
        "use_class_weights": bool(args.use_class_weights),
        "d1b": {
            "enabled": bool(float(args.lambda_pair) > 0.0),
            "lambda_pair": float(args.lambda_pair),
            "aux_hidden_dim": int(args.aux_hidden_dim),
            "aux_dropout": float(args.aux_dropout),
            "hard_pairs": D1A_HARD_PAIRS,
            "note": "D1b fork keeps official 07_train loop; only train_one_epoch is replaced when lambda_pair > 0.",
        },
    ...
    d1b_aux_info: Dict[str, object] = {"enabled": False}
    if float(args.lambda_pair) > 0.0:
        d1b_aux_info = attach_d1b_aux_heads(
            model,
            label_names=label_names,
            hidden_dim=int(args.hidden_dim),
            aux_hidden_dim=int(args.aux_hidden_dim),
        ...
        # Important: aux heads/queries are attached after model creation; move them to selected device.
        model.to(device)
        d1b_aux_info["enabled"] = True
        print(f"D1b auxiliary heads enabled: lambda_pair={float(args.lambda_pair)} info={d1b_aux_info}")
    else:
        print("D1b auxiliary heads disabled: lambda_pair=0.0, exact official train_one_epoch path")

    config_obj["d1b"]["aux_info"] = d1b_aux_info
    ...
    # Ensure all modules, including D1b aux heads attached after model creation, are on the selected device.
    model.to(device)
    ...
    best_val_eval = None
    best_aux_pair_eval: List[Dict[str, object]] = []
    bad_epochs = 0
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if float(args.lambda_pair) > 0.0:
            train_step = train_one_epoch_d1b(
                model=model,
                loader=train_loader,
                criterion=criterion,
                optimizer=optimizer,
            ...
            train_step_loss = float(train_step["loss"])
            train_step_main_loss = float(train_step["main_loss"])
            train_step_pair_loss = float(train_step["pair_loss"])
        else:
            ...
            train_step_main_loss = float(train_step_loss)
            train_step_pair_loss = 0.0
        ...
        row = {
            ...
            "train_step_main_loss": float(train_step_main_loss),
            "train_step_pair_loss": float(train_step_pair_loss),
    ...
    _train_mod.save_json(out_dir / "d1b_aux_pair_metrics_best.json", best_aux_pair_eval)
    if best_aux_pair_eval:
        pd.DataFrame(best_aux_pair_eval).to_csv(out_dir / "d1b_aux_pair_metrics_best.csv", index=False)
    ...
    diagnosis["d1b"] = {
        ...
        "aux_hidden_dim": int(args.aux_hidden_dim),
        "aux_dropout": float(args.aux_dropout),
        "aux_pair_metrics_best": best_aux_pair_eval,
        "guardrail": "Inference uses main 4-class logits only; auxiliary pair heads are training losses, not post-hoc rerankers.",
    }
