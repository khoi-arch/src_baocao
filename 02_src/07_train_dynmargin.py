...
def parse_args() -> argparse.Namespace:
    ...
    # F3c: dynamic hard-negative margin. No fixed hard-pair list, no L3/family labels.
    p.add_argument("--dyn-margin-lambda", type=float, default=0.0, help="Weight for dynamic hard-negative margin loss. 0 disables.")
    p.add_argument("--dyn-margin-margin", type=float, default=0.2, help="Required logit margin: logit_true - max_wrong_logit >= margin.")
    p.add_argument("--dyn-margin-scope", default="all", choices=["all", "malware"], help="all = all classes; malware = only true/confuser in Ransomware/Spyware/Trojan.")
    p.add_argument("--dyn-margin-reduction", default="mean_all", choices=["mean_all", "mean_active"], help="mean_all averages over batch; mean_active averages over active violations.")
...
class DynamicHardNegativeMarginLoss(nn.Module):
    """
    CE + lambda * ReLU(margin + max_wrong_logit - true_logit).

    This is NOT fixed hard-pair coding.
    For each sample, the confuser is selected dynamically from current logits:
        hard_negative = argmax_{c != y} logit_c
    ...
    def __init__(
        ...
        *,
        ...
        if self.scope not in {"all", "malware"}:
            raise ValueError(f"bad dyn margin scope: {self.scope}")
        if self.reduction not in {"mean_all", "mean_active"}:
            raise ValueError(f"bad dyn margin reduction: {self.reduction}")
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        if self.scope == "malware":
            if not self.malware_ids:
                raise ValueError("dyn-margin-scope=malware requires label_names containing Ransomware/Spyware/Trojan.")
            ...
            # rows whose true label is not malware should not receive aux loss
            wrong_logits = wrong_logits.masked_fill(~valid_mask, -1e9)
        else:
            true_is_malware = torch.ones(B, dtype=torch.bool, device=logits.device)
            wrong_logits = wrong_logits.masked_fill(~valid_mask, -1e9)

        max_wrong_logits, hard_neg_ids = wrong_logits.max(dim=1)
        raw_violation = float(self.margin) + max_wrong_logits - true_logits
        violation = F.relu(raw_violation)

        # If malware scope and true label not malware, zero out aux loss.
        if self.scope == "malware":
            violation = violation * true_is_malware.to(dtype=violation.dtype)

        active = violation > 0
        if self.reduction == "mean_active":
            denom = active.to(dtype=violation.dtype).sum().clamp_min(1.0)
            margin_loss = violation.sum() / denom
        else:
        ...
        return loss, {
            "base_loss": float(base_loss.detach().cpu()),
            "dyn_margin_loss": float(margin_loss.detach().cpu()),
            "active_rate": float(active.float().mean().detach().cpu()),
            "mean_margin_violation": float(violation.detach().mean().cpu()),
        }


def train_one_epoch_dynamic_margin(
    *,
    model: nn.Module,
    loader: DataLoader,
    criterion: DynamicHardNegativeMarginLoss,
...
def main() -> None:
    ...
    dyn_margin_info = {
        "enabled": bool(float(args.dyn_margin_lambda) > 0.0),
        "lambda": float(args.dyn_margin_lambda),
        "margin": float(args.dyn_margin_margin),
        "scope": str(args.dyn_margin_scope),
        "reduction": str(args.dyn_margin_reduction),
        "uses_fixed_hard_pairs": False,
        "uses_l3_or_family_labels": False,
        "uses_validation_to_select_pairs": False,
        "note": "Dynamic hard-negative is selected online from current logits: max wrong class per sample.",
    }
    config_obj["dynamic_hard_negative_margin"] = dyn_margin_info
    _train_mod.save_json(out_dir / "dynamic_margin_info.json", dyn_margin_info)
    _train_mod.save_json(out_dir / "config.json", config_obj)
    ...
    elif float(args.dyn_margin_lambda) > 0.0:
        criterion = DynamicHardNegativeMarginLoss(
            eval_criterion,
            aux_lambda=float(args.dyn_margin_lambda),
            margin=float(args.dyn_margin_margin),
            scope=str(args.dyn_margin_scope),
            reduction=str(args.dyn_margin_reduction),
            label_names=label_names,
    ...
    print(f"dynamic hard-negative margin:      {float(args.dyn_margin_lambda) > 0.0} lambda={float(args.dyn_margin_lambda)} margin={float(args.dyn_margin_margin)} scope={str(args.dyn_margin_scope)} reduction={str(args.dyn_margin_reduction)}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if float(args.dyn_margin_lambda) > 0.0:
            if soft_targets_train is not None:
                raise ValueError("Do not combine soft targets with dynamic hard-negative margin in this experiment.")
            train_step_loss = train_one_epoch_dynamic_margin(
                model=model,
                loader=train_loader,
                criterion=criterion,
                optimizer=optimizer,
