...
def parse_args() -> argparse.Namespace:
    ...
    # F3b: OOF-selected hard pair/family margin repair.
    p.add_argument("--hard-margin-groups-json", default="", help="Path to F3a2_train_only_selected_hard_groups.json.")
    p.add_argument("--hard-margin-lambda", type=float, default=0.0, help="Weight for OOF-selected hard-pair margin loss. 0 disables.")
    p.add_argument("--hard-margin-margin", type=float, default=0.2, help="Required logit margin: logit_true - logit_confuser >= margin.")
    p.add_argument("--hard-margin-mode", default="pair_family", choices=["pair_family", "pair_family_plus_pair", "pair"], help="How to apply OOF selected hard groups.")
    p.add_argument("--hard-margin-family-col", default="", help="Optional raw CSV family column. Auto-detect if empty.")
...
class FusionHardMarginDataset(Dataset):
    """
    Returns:
      X_bin: [F]
      values: [F, 3]
      y
      confuser_mask: [C], 1 for selected confuser classes for this sample.
    """
    def __init__(
        self,
        X_bin: np.ndarray,
        ...
        if y.ndim != 1 or y.shape[0] != X_bin.shape[0]:
            raise ValueError(f"y mismatch: {y.shape} vs {X_bin.shape}")
        confuser_mask = np.asarray(confuser_mask, dtype=np.float32)
        if confuser_mask.ndim != 2 or confuser_mask.shape[0] != X_bin.shape[0]:
            raise ValueError(f"confuser_mask mismatch: {confuser_mask.shape} vs rows={X_bin.shape[0]}")

        vals = np.stack([X_offset.astype(np.float32), X_cont.astype(np.float32), X_mask.astype(np.float32)], axis=-1)
        self.X = torch.as_tensor(X_bin, dtype=torch.long)
        self.V = torch.as_tensor(vals, dtype=torch.float32)
        self.y = torch.as_tensor(y, dtype=torch.long)
        self.confuser_mask = torch.as_tensor(confuser_mask, dtype=torch.float32)

    def __len__(self) -> int:
        return int(self.X.shape[0])

    def __getitem__(self, idx: int):
        return self.X[idx], self.V[idx], self.y[idx], self.confuser_mask[idx]


def _find_family_col_for_hard_margin(df: pd.DataFrame, preferred: str = "") -> str | None:
...
def _clean_label_value(x) -> str:
    if pd.isna(x):
        return ""
    return str(x).strip()


def build_hard_margin_confuser_mask(
    *,
    train_raw_path: str | Path,
    y_train: np.ndarray,
    ...
    selected_path = Path(selected_json_path)
    if not selected_path.exists():
        raise FileNotFoundError(f"hard-margin selected groups json not found: {selected_path}")

    obj = json.loads(selected_path.read_text(encoding="utf-8"))
    if bool(obj.get("validation_used", False)):
        raise ValueError("Selected hard groups json says validation_used=true; refusing to use it.")

    name_to_id = {str(c): int(i) for i, c in enumerate(label_names)}
    n = int(len(y_train))
    c = int(len(label_names))
    mask = np.zeros((n, c), dtype=np.float32)

    raw = pd.read_csv(train_raw_path)
    if len(raw) != n:
        raise ValueError(f"train_raw rows {len(raw)} != y_train rows {n}")

    fam_col = _find_family_col_for_hard_margin(raw, preferred=str(family_col or ""))
    if fam_col is None:
        families = np.asarray([label_names[int(i)] for i in y_train], dtype=object)
    else:
        families = raw[fam_col].map(_clean_label_value).to_numpy()
        fallback = np.asarray([label_names[int(i)] for i in y_train], dtype=object)
        families = np.where(pd.Series(families).astype(str).str.len().to_numpy() > 0, families, fallback)

    pair_family_map: Dict[Tuple[str, str], set[str]] = {}
    pair_map: Dict[str, set[str]] = {}

    for r in obj.get("hard_pair_families", []):
        true_lab = str(r.get("true_label", "")).strip()
        pred_lab = str(r.get("pred_label", "")).strip()
        fam = str(r.get("family", "")).strip()
        if true_lab in name_to_id and pred_lab in name_to_id and fam:
            pair_family_map.setdefault((true_lab, fam), set()).add(pred_lab)

    for r in obj.get("hard_pairs", []):
        true_lab = str(r.get("true_label", "")).strip()
        pred_lab = str(r.get("pred_label", "")).strip()
        if true_lab in name_to_id and pred_lab in name_to_id:
            pair_map.setdefault(true_lab, set()).add(pred_lab)
    ...
    for i in range(n):
        true_lab = label_names[int(y_train[i])]
        fam = str(families[i])
        confs: set[str] = set()
        if str(mode) in {"pair_family", "pair_family_plus_pair"}:
            confs |= pair_family_map.get((true_lab, fam), set())
        if str(mode) == "pair":
            confs |= pair_map.get(true_lab, set())
        if str(mode) == "pair_family_plus_pair" and not confs:
            confs |= pair_map.get(true_lab, set())

        confs = {x for x in confs if x in name_to_id and x != true_lab}
        if confs:
            active_rows += 1
            per_group_counts[f"{true_lab}::{fam}"] = per_group_counts.get(f"{true_lab}::{fam}", 0) + 1
        for conf in confs:
            mask[i, name_to_id[conf]] = 1.0
            active_entries += 1
    ...
    info = {
        ...
        "validation_used_in_selection": bool(obj.get("validation_used", None)),
        "mode": str(mode),
        "family_col": fam_col,
        "n_rows": n,
        "num_classes": c,
        "active_rows": int(active_rows),
        "active_row_rate": float(active_rows / max(n, 1)),
        "active_entries": int(active_entries),
        "pair_family_rules": int(sum(len(v) for v in pair_family_map.values())),
        "pair_rules": int(sum(len(v) for v in pair_map.values())),
        "top_active_true_family_counts": [
            {"true_family": k, "count": int(v)}
            for k, v in sorted(per_group_counts.items(), key=lambda kv: kv[1], reverse=True)[:30]
        ],
        "criteria": obj.get("criteria", {}),
        "note": "OOF-selected train-only hard groups. Applied to training rows only as a logit margin loss.",
    }
    return mask, info


class HardPairFamilyMarginLoss(nn.Module):
    def __init__(self, base_criterion: nn.Module, *, aux_lambda: float, margin: float):
        super().__init__()
        self.base_criterion = base_criterion
        self.aux_lambda = float(aux_lambda)
        self.margin = float(margin)

    def forward(self, logits: torch.Tensor, targets: torch.Tensor, confuser_mask: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        base_loss = self.base_criterion(logits, targets)
        confuser_mask = confuser_mask.to(device=logits.device, dtype=logits.dtype)
        active = confuser_mask.sum()
        if self.aux_lambda <= 0.0 or float(active.detach().cpu()) <= 0.0:
            return base_loss, {
                "base_loss": float(base_loss.detach().cpu()),
                "margin_loss": 0.0,
                "active_entries": float(active.detach().cpu()),
            }

        true_logits = logits.gather(1, targets.view(-1, 1))
        margin_values = true_logits - logits
        per_entry = F.relu(float(self.margin) - margin_values) * confuser_mask
        margin_loss = per_entry.sum() / active.clamp_min(1.0)
        loss = base_loss + float(self.aux_lambda) * margin_loss
        return loss, {
            "base_loss": float(base_loss.detach().cpu()),
...
def train_one_epoch_hard_margin(
    *,
    model: nn.Module,
    loader: DataLoader,
    criterion: HardPairFamilyMarginLoss,
    ...
    for batch in loader:
        if len(batch) != 4:
            raise ValueError("hard-margin loader must return (tokens, values, y, confuser_mask)")
        tokens, values, y, confuser_mask = batch
        tokens = tokens.to(device, non_blocking=True)
        values = values.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)
        confuser_mask = confuser_mask.to(device, non_blocking=True)

        optimizer.zero_grad(set_to_none=True)
        logits = model(tokens, values)
        loss, _ = criterion(logits, y, confuser_mask)
        loss.backward()
...
def main() -> None:
    ...
    hard_margin_info = {
        "enabled": bool(float(args.hard_margin_lambda) > 0.0),
        "lambda": float(args.hard_margin_lambda),
        "margin": float(args.hard_margin_margin),
        "mode": str(args.hard_margin_mode),
        "groups_json": str(args.hard_margin_groups_json),
    }
    hard_confuser_mask_train = None
    if float(args.hard_margin_lambda) > 0.0:
        if soft_targets_train is not None:
            raise ValueError("Do not combine soft targets with F3b hard-pair margin loss.")
        train_raw_path_for_margin, _val_raw_unused = resolve_raw_paths(args)
        hard_confuser_mask_train, extra_hm_info = build_hard_margin_confuser_mask(
            train_raw_path=train_raw_path_for_margin,
            y_train=y_train,
            label_names=label_names,
            selected_json_path=str(args.hard_margin_groups_json),
            mode=str(args.hard_margin_mode),
            family_col=str(args.hard_margin_family_col),
        )
        hard_margin_info.update(extra_hm_info)

    config_obj["hard_pair_margin"] = hard_margin_info
    _train_mod.save_json(out_dir / "hard_pair_margin_info.json", hard_margin_info)
    _train_mod.save_json(out_dir / "config.json", config_obj)

    if hard_confuser_mask_train is not None:
        train_ds = FusionHardMarginDataset(X_train, O_train, X_train_cont, M_train, y_train, hard_confuser_mask_train)
    else:
    ...
    elif float(args.hard_margin_lambda) > 0.0:
        criterion = HardPairFamilyMarginLoss(
            eval_criterion,
            aux_lambda=float(args.hard_margin_lambda),
            margin=float(args.hard_margin_margin),
        )
    else:
    ...
    print(f"hard-pair margin:                  {float(args.hard_margin_lambda) > 0.0} lambda={float(args.hard_margin_lambda)} margin={float(args.hard_margin_margin)} mode={str(args.hard_margin_mode)} active_rows={hard_margin_info.get('active_rows', 0)}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if float(args.hard_margin_lambda) > 0.0:
            train_step_loss = train_one_epoch_hard_margin(
                model=model,
                loader=train_loader,
                criterion=criterion,
                optimizer=optimizer,
