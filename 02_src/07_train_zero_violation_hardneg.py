...
def parse_args() -> argparse.Namespace:
    ...
    # F3g: zero-violation online hard-negative violation loss.
    # No fixed hard-pair list, no L3/family label, no alpha, no margin.
    p.add_argument("--zmh-enabled", action=argparse.BooleanOptionalAction, default=False, help="Enable zero-violation online hard-negative violation loss.")
...
class ZeroMarginOnlineHardNegativeLoss(nn.Module):
    """
    CE + online zero-violation violation loss.

    For each sample:
      true_logit = logit_y
      wrong logits = logits for c != y
      violation_c = ReLU(logit_c - true_logit)

    Only wrong classes that currently exceed the true class contribute.
    Correct-but-hard samples whose true_logit is still above all wrong logits
    receive zero auxiliary loss.

    No hard-pair list.
    No L3/family label.
    No fixed margin.
    No global alpha for auxiliary strength.
    """
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        wrong_logits_for_weight = logits.masked_fill(~valid_mask, -1e9)
        wrong_weights = torch.softmax(wrong_logits_for_weight, dim=1)

        violations = F.relu(logits - true_logits).masked_fill(~valid_mask, 0.0)
        aux_per_sample = (wrong_weights * violations).sum(dim=1)
        aux_loss = aux_per_sample.mean()

        loss = base_loss + aux_loss
        active_rate = (aux_per_sample > 0).float().mean()
        return loss, {
            "base_loss": float(base_loss.detach().cpu()),
            "zmh_aux_loss": float(aux_loss.detach().cpu()),
            "zmh_active_rate": float(active_rate.detach().cpu()),
            "zmh_aux_mean": float(aux_per_sample.detach().mean().cpu()),
        }


def train_one_epoch_zmh(
    *,
    model: nn.Module,
    loader: DataLoader,
    criterion: ZeroMarginOnlineHardNegativeLoss,
...
def main() -> None:
    ...
    zmh_info = {
        "enabled": bool(args.zmh_enabled),
        "uses_fixed_hard_pairs": False,
        "uses_l3_or_family_labels_for_training": False,
        "uses_fixed_margin": False,
        "uses_global_aux_alpha": False,
        "formula": "CE + mean(sum_wrong softmax(wrong_logits) * ReLU(logit_wrong - logit_true))",
        "note": "Auxiliary loss is zero for samples whose true logit already exceeds all wrong logits.",
    }
    config_obj["zero_violation_online_hard_negative"] = zmh_info
    _train_mod.save_json(out_dir / "zero_margin_hardneg_info.json", zmh_info)
    _train_mod.save_json(out_dir / "config.json", config_obj)
    ...
    elif bool(args.zmh_enabled):
        criterion = ZeroMarginOnlineHardNegativeLoss(eval_criterion)
    else:
    ...
    print(f"zero-violation online hardneg:        {bool(args.zmh_enabled)}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if bool(args.zmh_enabled):
            if soft_targets_train is not None:
                raise ValueError("Do not combine soft targets with zero-violation hard-negative loss.")
            if float(args.sam_rho) > 0.0:
                raise ValueError("Do not combine SAM with zero-violation hard-negative loss.")
            train_step_loss = train_one_epoch_zmh(
                model=model,
                loader=train_loader,
                criterion=criterion,
                optimizer=optimizer,
