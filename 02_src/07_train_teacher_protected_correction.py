...
def parse_args() -> argparse.Namespace:
    ...
    # F4a: teacher-protected correction.
    # Teacher is a baseline CE model trained on the same train split.
    # Teacher-correct samples are protected by KL; teacher-wrong samples receive zero-violation correction.
    p.add_argument("--tpc-enabled", action=argparse.BooleanOptionalAction, default=False, help="Enable teacher-protected correction loss.")
    p.add_argument("--tpc-teacher-checkpoint", default="", help="Path to baseline teacher best_model.pt.")
    p.add_argument("--tpc-temperature", type=float, default=1.0, help="Temperature for teacher KL protection.")
...
class TeacherProtectedCorrectionLoss(nn.Module):
    """
    F4a teacher-protected correction.

    Partition is defined by a frozen teacher trained with the baseline CE recipe
    on the same train split:
      - teacher-correct samples: protect student distribution by KL(teacher || student)
      - teacher-wrong samples: apply zero-violation hard-negative correction

    This directly targets the F3g failure:
      F3g corrected wrong samples but moved shared boundary and damaged
      teacher/baseline-correct Ransomware samples.

    No fixed hard-pair list.
    No L3/family label.
    No official validation signal.
    No fixed margin m>0.
    No alpha/grid search. Fixed objective: CE + correction + protection.
    """
    def __init__(self, base_criterion: nn.Module, *, temperature: float = 1.0):
        super().__init__()
    ...
    def forward(
        ...
        *,
        ...
        with torch.no_grad():
            ...
            teacher_correct = (teacher_pred == targets).to(dtype=student_logits.dtype)
            teacher_wrong = 1.0 - teacher_correct

        B, C = student_logits.shape
        true_logits = student_logits.gather(1, targets.view(-1, 1))
        valid_mask = torch.ones((B, C), dtype=torch.bool, device=student_logits.device)
        valid_mask.scatter_(1, targets.view(-1, 1), False)

        wrong_logits_for_weight = student_logits.masked_fill(~valid_mask, -1e9)
        wrong_weights = torch.softmax(wrong_logits_for_weight, dim=1)
        violations = F.relu(student_logits - true_logits).masked_fill(~valid_mask, 0.0)
        correction_per_sample = (wrong_weights * violations).sum(dim=1)
        correction_loss = (correction_per_sample * teacher_wrong).mean()

        T = self.temperature
        student_logp_T = F.log_softmax(student_logits / T, dim=1)
        with torch.no_grad():
            teacher_p_T = F.softmax(teacher_logits / T, dim=1)
        kl_per_sample = F.kl_div(student_logp_T, teacher_p_T, reduction="none").sum(dim=1) * (T * T)
        protection_loss = (kl_per_sample * teacher_correct).mean()

        total = base_loss + correction_loss + protection_loss

        with torch.no_grad():
            active_correction_rate = ((correction_per_sample > 0).to(dtype=student_logits.dtype) * teacher_wrong).mean()
            teacher_correct_rate = teacher_correct.mean()
            teacher_wrong_rate = teacher_wrong.mean()
        ...
        return total, {
            ...
            "teacher_correct_rate": float(teacher_correct_rate.detach().cpu()),
            "teacher_wrong_rate": float(teacher_wrong_rate.detach().cpu()),
            "active_correction_rate": float(active_correction_rate.detach().cpu()),
        }


def load_teacher_model_for_tpc(
    *,
    checkpoint_path: str,
    device: torch.device,
    run_id: str,
    ...
    if not checkpoint_path:
        raise ValueError("--tpc-teacher-checkpoint is required when --tpc-enabled")
    ...
    teacher = FusionAblationTransformer(
        run_id=run_id,
        num_bins=num_bins,
    ...
    if missing or unexpected:
        print(f"[F4a teacher load] missing={missing} unexpected={unexpected}", flush=True)
    teacher.eval()
...
def train_one_epoch_teacher_protected(
    *,
    model: nn.Module,
    teacher_model: nn.Module,
    loader: DataLoader,
    criterion: TeacherProtectedCorrectionLoss,
    ...
    for batch in loader:
        ...
        student_logits = model(tokens, values)
        with torch.no_grad():
            teacher_logits = teacher_model(tokens, values)
...
def main() -> None:
    ...
    tpc_info = {
        "enabled": bool(args.tpc_enabled),
        "teacher_checkpoint": str(args.tpc_teacher_checkpoint),
        "temperature": float(args.tpc_temperature),
        "uses_fixed_hard_pairs": False,
        "uses_l3_or_family_labels_for_training": False,
        "uses_official_validation_for_training_signal": False,
        "uses_fixed_margin_gt_zero": False,
        "uses_alpha_grid": False,
        "formula": "CE + teacher_wrong * zero_violation_correction + teacher_correct * KL(teacher||student)",
        "correction": "sum_wrong softmax(student_wrong_logits) * ReLU(student_wrong_logit - student_true_logit), only for teacher-wrong samples",
        "protection": "KL teacher distribution to student, only for teacher-correct samples",
    }
    config_obj["teacher_protected_correction"] = tpc_info
    _train_mod.save_json(out_dir / "teacher_protected_correction_info.json", tpc_info)
    _train_mod.save_json(out_dir / "config.json", config_obj)
    ...
    teacher_model = None
    if bool(args.tpc_enabled):
        teacher_model = load_teacher_model_for_tpc(
            checkpoint_path=str(args.tpc_teacher_checkpoint),
            device=device,
            run_id=args.run_id,
            num_bins=B,
            n_features=n_features,
    ...
    elif bool(args.tpc_enabled):
        criterion = TeacherProtectedCorrectionLoss(eval_criterion, temperature=float(args.tpc_temperature))
    else:
    ...
    print(f"teacher-protected correction:      {bool(args.tpc_enabled)} teacher={str(args.tpc_teacher_checkpoint)} temp={float(args.tpc_temperature)}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if bool(args.tpc_enabled):
            if soft_targets_train is not None:
                raise ValueError("Do not combine soft targets with teacher-protected correction.")
            if float(args.sam_rho) > 0.0:
                raise ValueError("Do not combine SAM with teacher-protected correction.")
            if teacher_model is None:
                raise RuntimeError("teacher_model is None while tpc is enabled")
            train_step_loss = train_one_epoch_teacher_protected(
                model=model,
