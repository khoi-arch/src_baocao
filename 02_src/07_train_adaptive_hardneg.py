...
def parse_args() -> argparse.Namespace:
    ...
    # F3d: adaptive hard-negative separation. No fixed hard-pair list, no L3/family labels.
    p.add_argument("--ahn-alpha", type=float, default=0.0, help="Weight for adaptive hard-negative separation loss. 0 disables.")
    p.add_argument("--ahn-temperature", type=float, default=1.0, help="Softmax temperature over wrong-class logits for adaptive weights.")
    p.add_argument("--ahn-scope", default="all", choices=["all", "malware"], help="all = all classes; malware = only Ransomware/Spyware/Trojan true/confuser classes.")
    p.add_argument("--ahn-detach-weights", action=argparse.BooleanOptionalAction, default=True, help="Detach adaptive wrong-class weights from gradient.")
...
class AdaptiveHardNegativeSeparationLoss(nn.Module):
    """
    CE + alpha * adaptive hard-negative separation.

    For every sample:
      true_logit = logit[y]
      wrong_logit_c = logit[c], c != y
      weight_c = softmax(wrong_logit_c / temperature) over wrong classes
      sep_c = softplus(wrong_logit_c - true_logit)

      aux = sum_c weight_c * sep_c

    Interpretation:
      - no fixed hard pair list
      - no L3/family label
      - no margin hyperparameter
      - wrong classes that are currently more dangerous receive larger weight automatically
      - severity is also automatic via softplus(wrong - true)

    The only global strength knob is alpha, like weight decay/dropout/focal-gamma.
    """
    def __init__(
        self,
        ...
        *,
        ...
        if self.temperature <= 0:
            raise ValueError("ahn-temperature must be > 0")
        if self.scope not in {"all", "malware"}:
            raise ValueError(f"bad ahn scope: {self.scope}")

        malware_names = {"Ransomware", "Spyware", "Trojan"}
        self.malware_ids = [i for i, name in enumerate(self.label_names) if str(name) in malware_names]
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        true_logits = logits.gather(1, targets.view(-1, 1))  # [B,1]
        wrong_mask = torch.ones((B, C), dtype=torch.bool, device=device)
        wrong_mask.scatter_(1, targets.view(-1, 1), False)

        row_active = torch.ones(B, dtype=torch.bool, device=device)
        if self.scope == "malware":
            if not self.malware_ids:
                raise ValueError("ahn-scope=malware requires label_names with Ransomware/Spyware/Trojan.")
            malware_ids = torch.as_tensor(self.malware_ids, dtype=torch.long, device=device)
            class_is_malware = torch.zeros(C, dtype=torch.bool, device=device)
            class_is_malware[malware_ids] = True
            true_is_malware = class_is_malware[targets]
            wrong_mask = wrong_mask & class_is_malware.view(1, C)
            row_active = true_is_malware

        masked_wrong_logits = logits.masked_fill(~wrong_mask, -1e9)
        weights = torch.softmax(masked_wrong_logits / float(self.temperature), dim=1)
        weights = weights.masked_fill(~wrong_mask, 0.0)

        # Re-normalize in case masking removed classes.
        weights = weights / weights.sum(dim=1, keepdim=True).clamp_min(1e-12)
        if self.detach_weights:
            weights_for_loss = weights.detach()
        else:
            weights_for_loss = weights

        sep = F.softplus(logits - true_logits)  # [B,C], positive if wrong logit approaches/exceeds true logit
        sep = sep.masked_fill(~wrong_mask, 0.0)
        per_sample = (weights_for_loss * sep).sum(dim=1)

        per_sample = per_sample * row_active.to(dtype=dtype)
        aux_loss = per_sample.mean()

        loss = base_loss + float(self.alpha) * aux_loss

        # entropy of wrong-class weights: low = one dominant hard negative; high = multiple competitors.
        entropy = -(weights.clamp_min(1e-12) * weights.clamp_min(1e-12).log()).sum(dim=1)
        entropy = entropy * row_active.to(dtype=dtype)

        return loss, {
            "base_loss": float(base_loss.detach().cpu()),
            "ahn_loss": float(aux_loss.detach().cpu()),
            "ahn_active_rate": float(row_active.float().mean().detach().cpu()),
            "ahn_mean_weight_entropy": float(entropy.detach().mean().cpu()),
        }


def train_one_epoch_adaptive_hardneg(
    *,
    model: nn.Module,
    loader: DataLoader,
    criterion: AdaptiveHardNegativeSeparationLoss,
    ...
    for batch in loader:
        ...
        loss, _stats = criterion(logits, y)
        loss.backward()
...
def main() -> None:
    ...
    adaptive_hardneg_info = {
        "enabled": bool(float(args.ahn_alpha) > 0.0),
        "alpha": float(args.ahn_alpha),
        "temperature": float(args.ahn_temperature),
        "scope": str(args.ahn_scope),
        "detach_weights": bool(args.ahn_detach_weights),
        "uses_fixed_hard_pairs": False,
        "uses_l3_or_family_labels": False,
        "uses_validation_to_select_pairs": False,
        "has_fixed_margin": False,
        "formula": "CE + alpha * sum_wrong softmax(logit_wrong/T) * softplus(logit_wrong - logit_true)",
        "note": "Wrong-class weights and severity are computed online from current logits.",
    }
    config_obj["adaptive_hard_negative_separation"] = adaptive_hardneg_info
    _train_mod.save_json(out_dir / "adaptive_hardneg_info.json", adaptive_hardneg_info)
    _train_mod.save_json(out_dir / "config.json", config_obj)
    ...
    print(f"adaptive hard-negative separation: {float(args.ahn_alpha) > 0.0} alpha={float(args.ahn_alpha)} T={float(args.ahn_temperature)} scope={str(args.ahn_scope)} detach_weights={bool(args.ahn_detach_weights)}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if float(args.ahn_alpha) > 0.0:
            if soft_targets_train is not None:
                raise ValueError("Do not combine soft targets with adaptive hard-negative loss in this experiment.")
            train_step_loss = train_one_epoch_adaptive_hardneg(
                model=model,
                loader=train_loader,
                criterion=criterion,
                optimizer=optimizer,
